...
unsigned long LLPublishPeriod = 12000;
unsigned long LLPublishSampleTime;

unsigned long TempPublishPeriod = 8000;
...
boolean registerT1, registerT2, registerT3, registerT4, registerT5, registerT6,
        subscribeCycle, allSubscribed, delayedGwInfo, advertiseKey, 
        allRegistered, registerCycle;
...
char* topic4 = "Lifetime_walking_miles";
uint16_t topic4Id;

char* topic5 = "Lifetime_running_miles";
uint16_t topic5Id;

char* topic6 = "Walkink_step_counter";
uint16_t topic6Id;

char* topic7 = "Running_step_counter";
uint16_t topic7Id;
...
SoftwareSerial nikePlusReceiver(2,3);  //RX=2, TX=3
    ...
    case DISCONNECTED:
      if(millis() - lastAdvertiseTime > tAdvertise && advertiseKey) {
        ...
        if(missedadvertise > N_ADVERTISE) {
          Serial.print("\ncr s-D ma-"); Serial.println(missedadvertise);
          state = IDLE;
    ...
    case CONNECTED:
      if(!subscribeCycle & !allSubscribed) { 
        ...
        if(!registerT1) {
          ...
          Serial.println("\ns-C rt4");
          sprintf(topicBuffer, "sensors/%s/%s", clientId, topic4);
        }
        else if(!registerT4) {
          Serial.println("\ns-C rt5");
          sprintf(topicBuffer, "sensors/%s/%s", clientId, topic5);
        }
        else if(!registerT5) {
          Serial.println("\ns-C rt6");
          sprintf(topicBuffer, "sensors/%s/%s", clientId, topic6);
        }
        else if(!registerT6) {
          Serial.println("\ns-C rt7");
          sprintf(topicBuffer, "sensors/%s/%s", clientId, topic7);
          allRegistered = true;
        }
        ...
        missedadvertise++;
        if(missedadvertise > N_ADVERTISE) {
          Serial.print("\ncr s-C ma-"); Serial.println(missedadvertise);
          state = IDLE;
          digitalWrite(gwLed, LOW);
          digitalWrite(connectLed, LOW);
          digitalWrite(registerLed, LOW);
      ...
      else if(nikePlusReceiver.available()) {
        readNikePlusPacket();
      }
      break; 
    default: 
  ...
  void readNikePlusPacket() {
    if(nikePlusReceiver.available()>0) {
      ...
      for(int i=0;i<sizeof(data);i++) {
        ...
        while(nikePlusReceiver.available()==0 && !timeout)
          timeout = int((millis() - start)) > 100;
      ...
      dtostrf(walkingMiles, 7, 4, floatBuf);  // http://dereenigne.org/arduino/arduino-float-to-string
      Serial.print("Lifetime Walking Miles: ");
      Serial.println(floatBuf);
      publish(0, topic4Id, MsgId, floatBuf);    

      
      float runningMiles=(64*getLifetimeRunningMiles(payload))/18947.368115186691f;
      dtostrf(runningMiles, 7, 4, floatBuf);  // http://dereenigne.org/arduino/arduino-float-to-string
      Serial.print("Lifetime Running Miles: ");
      Serial.println(floatBuf);
      publish(0, topic5Id, MsgId, floatBuf);    
      
      uint32_t WalkingStepCount=(getWalkingStepCount(payload));
      dtostrf(WalkingStepCount, 7, 0, floatBuf);  // http://dereenigne.org/arduino/arduino-float-to-string
      Serial.print("Walking Step Count: ");
      Serial.println(WalkingStepCount);
      publish(0, topic6Id, MsgId, floatBuf);

      uint32_t RunningStepCount=(getRunningStepCount(payload));
      dtostrf(RunningStepCount, 7, 0, floatBuf);
      Serial.print("Running Step Count: ");
      Serial.println(RunningStepCount);
      publish(0, topic7Id, MsgId, floatBuf);
    }
  }
    ...
    switch(returnCode) {
      case MQTTS_RC_ACCEPTED:
        if(!registerT1) {
          ...
          topic4Id = regAck.getTopicId();
          registerT3 = true;
        }else if(!registerT4) {
          topic5Id = regAck.getTopicId();
          registerT4 = true; 
        }else if(!registerT5) {
          topic6Id = regAck.getTopicId();
          registerT5 = true;
        }else if(!registerT6) {
          topic7Id = regAck.getTopicId();
          registerT6 = true;
          LLPublishSampleTime = millis();
          TempPublishSampleTime = millis();
          pingRequestSampleTime = millis();
